...
USAGE="run-tests-with-remote-controller.sh --client-version=<clientversion> --server-version=<serverversion> --build-name=<unique build name for_logs>"
...
java -cp "${CLASSPATH}" -Dhazelcast.enterprise.license.key=${HAZELCAST_ENTERPRISE_KEY} com.hazelcast.remotecontroller.Main > ${SERVER_LOG_FILE} 2>${SERVER_ERR_FILE} &
serverPid=$!

echo "Spawned remote controller with pid ${serverPid}"
...
echo "Starting tests for client version:${CLIENT_VERSION} against server version:${SERVER_VERSION}. Build name: ${BUILD_NAME}"

CLIENT_LOG_FILE=client-${CLIENT_VERSION}-server-${SERVER_VERSION}-out.log
CLIENT_ERR_FILE=client-${CLIENT_VERSION}-server-${SERVER_VERSION}-err.log
...
if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
...
if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]]|| [[ $SERVER_VERSION == 3.8* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
        TEST_FILTER="--gtest_filter=-ClientStatisticsTest.*"
     else
        TEST_FILTER+=":ClientStatisticsTest.*"
     fi
fi

if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]] || [[ $SERVER_VERSION == 3.8* ]] || [[ $SERVER_VERSION == 3.9* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
        TEST_FILTER="--gtest_filter=-ClientReplicatedMapTest.testRetrieveUnknownValue"
     else
        TEST_FILTER+=":ClientReplicatedMapTest.testRetrieveUnknownValue"
     fi
fi
...
if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]] || [[ $SERVER_VERSION == 3.8* ]] || [[ $SERVER_VERSION == 3.9* ]] || [[ $SERVER_VERSION == 3.10* ]] || [[ $SERVER_VERSION == 3.11* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
        TEST_FILTER="--gtest_filter=-*test*Json*"
     else
        TEST_FILTER+=":*test*Json*"
     fi
fi

if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]] || [[ $SERVER_VERSION == 3.8* ]] || [[ $SERVER_VERSION == 3.9* ]] || [[ $SERVER_VERSION == 3.10* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
        TEST_FILTER="--gtest_filter=-*testAsyncPutWithMaxIdle*"
     else
        TEST_FILTER+=":*testAsyncPutWithMaxIdle*"
     fi
fi

if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]] || [[ $SERVER_VERSION == 3.8* ]] || [[ $SERVER_VERSION == 3.9* ]] || [[ $SERVER_VERSION == 3.10* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
        TEST_FILTER="--gtest_filter=-*testAsyncSetWithMaxIdle*"
     else
        TEST_FILTER+=":*testAsyncSetWithMaxIdle*"
     fi
fi

if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]] || [[ $SERVER_VERSION == 3.8* ]] || [[ $SERVER_VERSION == 3.9* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
        TEST_FILTER="--gtest_filter=-ClientExecutorServiceTest.testUnserializableResponse_exceptionPropagatesToClient*"
     else
        TEST_FILTER+=":ClientExecutorServiceTest.testUnserializableResponse_exceptionPropagatesToClient*"
     fi
fi

if [[ $SERVER_VERSION == 3.6* ]] || [[ $SERVER_VERSION == 3.7* ]] || [[ $SERVER_VERSION == 3.8* ]] || [[ $SERVER_VERSION == 3.9* ]]; then
     if [ "x${TEST_FILTER}" == "x" ]; then
        TEST_FILTER="--gtest_filter=-*testAllClientStatesWhenUserShutdown*"
     else
        TEST_FILTER+=":*testAllClientStatesWhenUserShutdown*"
     fi
fi
