...
class AzureEnterpriseClusterTestsWithSsl(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.cluster = cls.rc.createEnterpriseCluster("azure", os.getenv('hzVersion'), True)
    ...
    def test_connect_enterprise_ssl_cluster_with_certificates_unisocket(self):
        ...
        map1 = self.unisocketClient.get_map("map_for_test_connect_enterprise_ssl_cluster_with_certificates_unisocket").blocking()
        map1.clear()
        while map1.size() < 20:
            random_key = random.randint(1, 100000)
            try:
