...
typedef struct _CloudCluster__isset {
  _CloudCluster__isset() : id(false), name(false), nameForConnect(false), hazelcastVersion(false), isTlsEnabled(false), state(false), token(false), certificatePath(false), tlsPassword(false) {}
  bool id :1;
  bool name :1;
  bool nameForConnect :1;
  bool hazelcastVersion :1;
  bool isTlsEnabled :1;
  bool state :1;
  bool token :1;
  bool certificatePath :1;
  bool tlsPassword :1;
} _CloudCluster__isset;

class CloudCluster : public virtual ::apache::thrift::TBase {
 public:

  CloudCluster(const CloudCluster&);
  CloudCluster& operator=(const CloudCluster&);
  CloudCluster() : id(), name(), nameForConnect(), hazelcastVersion(), isTlsEnabled(0), state(), token(), certificatePath(), tlsPassword() {
  }

  virtual ~CloudCluster() noexcept;
  std::string id;
  std::string name;
  std::string nameForConnect;
  ...
  _CloudCluster__isset __isset;

  void __set_id(const std::string& val);

  void __set_name(const std::string& val);

  void __set_nameForConnect(const std::string& val);
  ...
  void __set_certificatePath(const std::string& val);

  void __set_tlsPassword(const std::string& val);

  bool operator == (const CloudCluster & rhs) const
  {
    if (!(id == rhs.id))
      return false;
    if (!(name == rhs.name))
      return false;
    if (!(nameForConnect == rhs.nameForConnect))
      return false;
    ...
    if (!(token == rhs.token))
      return false;
    if (!(certificatePath == rhs.certificatePath))
      return false;
    if (!(tlsPassword == rhs.tlsPassword))
      return false;
    return true;
  }
  bool operator != (const CloudCluster &rhs) const {
  ...
  bool operator < (const CloudCluster & ) const;
